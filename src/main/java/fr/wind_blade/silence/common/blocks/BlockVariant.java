...
public class BlockVariant<T extends Enum<T> & IStringSerializable> extends BlockGenericDecoration implements IVariantProvider {
	...
	public static <T extends Enum<T> & IStringSerializable> BlockVariant<T> create(Material material, Class<T> enumClass) {
		if(enumClass.getEnumConstants().length > 4) {
			throw new IllegalArgumentException("The number of variants for a block with rotation can't be higher then 4, create another block to have more");
		}
	...
	public String getVariantName(int variant) {
		return this.enumClass.getEnumConstants()[variant].getName();
	}
